...
-(NSString *)readSignature:(NSNumber *)handle pos:(NSNumber *)pos {
    ...
    [h seekToFileOffset:pos.intValue];
...
-(NSString *)readUTF8String:(NSNumber *)handle pos:(NSNumber *)pos size:(NSNumber *)size {
    NSFileHandle *h=[self handleByFid :handle.intValue];
    uint64_t p=pos.intValue;
    [h seekToFileOffset:p];
    uint64_t sz=size.intValue;
...
-(NSString *)readULE16String:(NSNumber *)handle pos:(NSNumber *)pos size:(NSNumber *)size {
    NSFileHandle *h=[self handleByFid :handle.intValue];
    [h seekToFileOffset:pos.intValue];
    NSData *data = [h readDataOfLength:size.intValue];

    NSString *str = [[NSString alloc] initWithData:data encoding:NSUTF16LittleEndianStringEncoding];
    return str;
}

-(NSArray *)readBuf:(NSNumber *)handle pos:(NSNumber *)pos size:(NSNumber *)size {
    NSFileHandle *h=[self handleByFid :handle.intValue];
    if (!h) return nil;
    ...
    NSMutableArray* out=[NSMutableArray arrayWithCapacity :size.intValue];
...
-(NSDictionary *)readBuf_packedint:(NSNumber *)handle pos:(NSNumber *)pos size:(NSNumber *)size  count:(NSNumber *)count  reset:(NSNumber *)reset {
    NSFileHandle *h=[self handleByFid :handle.intValue];    if (!h) return nil;

    
    uint64_t p=pos.intValue;
    [h seekToFileOffset:p];
    uint32_t sz=size.intValue;
    NSData *data = [h readDataOfLength:sz];
    

    uint8_t * c = (uint8_t*)([data bytes]);
    NSDictionary *r=[self unpack_int_hex :c length:size.intValue count:count.intValue reset: (bool)reset.intValue ];
...
-(NSArray *)readFixedArray:(NSNumber *)handle pos:(NSNumber *)pos count:(NSNumber *)count unitsz:(NSNumber *)unitsz {
    ...
    [h seekToFileOffset:pos.intValue];
    uint32_t blocksize=unitsz.intValue * count.intValue;
    NSData *data = [h readDataOfLength :blocksize];

    NSMutableArray* out=[NSMutableArray arrayWithCapacity :count.intValue];

    uint32_t unit=unitsz.intValue;
    uint32_t cnt=count.intValue;
    int i;
    if (unit==1) {
...
-(NSString*) readStringArray:(NSNumber *)handle pos:(NSNumber *)pos  size:(NSNumber *)size enc:(NSString *)enc {
    NSString *s;

    if ( [enc isEqualToString:@"utf8"]) {
